...
function ProductCard({ produto }) {
  ...
  return (
    <div className="group bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-md)] overflow-hidden flex flex-col transition-all duration-300 hover:border-[var(--color-accent)]/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/30">
      <Link to={`/produto/${produto.id}`} className="contents">
      ...
      {/* botão "Adicionar ao carrinho" fica FORA do Link, senão o clique no botão também navegaria */}
      <div className="p-[var(--space-sm)] pt-0">
        <button
          onClick={() => adicionarItem(produto)}
          disabled={semEstoque}
          className={`w-full py-2 rounded-[var(--radius-sm)] text-sm font-semibold transition-all duration-200 ${
            semEstoque
