...
function CadastroProduto() {
  ...
  useEffect(() => {
    ...
    async function carregarProduto() {
      try {
        ...
        setMensagem({ tipo: 'erro', texto: 'Não foi possível carregar o produto.' });
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const mensagemErro = err.response?.data?.error || 'Não foi possível salvar o produto. Tente novamente.';
  ...
  if (carregandoProduto) {
    return <p className="text-[var(--color-text-secondary)]">Carregando produto...</p>;
  }
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-[var(--color-text-secondary)] mb-6">
        {modoEdicao ? 'Editar Produto' : 'Cadastro de Produtos'}
      </p>
      ...
      >
        ...
        <div className="flex gap-3 justify-end pt-2 border-t border-[var(--color-border)] mt-2">
          ...
          >
            {enviando ? 'Salvando...' : modoEdicao ? 'Atualizar Produto' : 'Salvar Produto'}
          </button>
        </div>
      </form>
    </div>
